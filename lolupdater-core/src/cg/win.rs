...
fn backup_cg() -> Result<()> {
    ...
    if cg_backup.exists() {
        ...
        update_file(&LOLP_GC_PATH.join("Cg.dll"), &cg_backup.join("Cg.dll"))?;
        update_file(&LOLP_GC_PATH.join("CgGL.dll"), &cg_backup.join("CgGL.dll"))?;
        update_file(
            &LOLP_GC_PATH.join("cgD3D9.dll"),
...
fn update_cg(cg_dir: &Path) -> Result<()> {
    update_file(&cg_dir.join("Cg.dll"), &LOLP_GC_PATH.join("Cg.dll"))?;
    update_file(&cg_dir.join("CgGL.dll"), &LOLP_GC_PATH.join("CgGL.dll"))?;
    update_file(&cg_dir.join("cgD3D9.dll"), &LOLP_GC_PATH.join("cgD3D9.dll"))?;

    update_file(&cg_dir.join("Cg.dll"), &LOLSLN_GC_PATH.join("Cg.dll"))?;
    update_file(&cg_dir.join("CgGL.dll"), &LOLSLN_GC_PATH.join("CgGL.dll"))?;
    update_file(&cg_dir.join("cgD3D9.dll"), &LOLSLN_GC_PATH.join("cgD3D9.dll"))?;
    Ok(())
}
